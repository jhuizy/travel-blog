...
const Post = ({ image, likes, comments, id }) =>
    <a target="_blank" href={`https://instagram.com/p/${id}`} className="block group overflow-hidden h-full relative z-0 w-1/2 lg:w-1/3 xl:w-1/4">
      <div className="m-4 overflow-hidden relative">
        <Img fluid={image} />
        <div className="absolute bg-black opacity-50 inset-0 z-0 w-full h-full group-hover:opacity-0 transition-all transition-200 transition-ease" />
        <div className="absolute text-white inset-0 z-10 w-full h-full flex flex-col justify-center items-center group-hover:opacity-0 transition-all transition-200 transition-ease">
